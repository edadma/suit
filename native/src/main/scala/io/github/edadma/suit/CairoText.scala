...
// 1×1 surface measures the same as the real one. It selects the face for the run's family and
// weight from the same `FontSet` the canvas paints with, so a string measures exactly as wide
// as it will draw.
final class CairoTextMeasurer(fonts: FontSet) extends TextMeasurer:
