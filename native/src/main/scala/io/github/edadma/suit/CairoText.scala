...
import io.github.edadma.libcairo.{Context, FontFace, Format, Surface, imageSurfaceCreate}
...
// 1×1 surface measures the same as the real one. It uses the same `FontFace` the canvas
// paints with, so a string measures exactly as wide as it will draw.
final class CairoTextMeasurer(fontFace: FontFace) extends TextMeasurer:
