...
// Raster (bitmap) image support rides the same seam as SVG. The decoder (stb_image) and Cairo
...
/** A decoded bitmap image, ready to paint. Held abstractly by the render tree and the DSL so
  ...
  * `Raster.fromBytes` on Native), which decodes JPEG/PNG/BMP/GIF/etc. through stb_image. Unlike
  * an SVG it has a definite pixel size — `width` x `height` — which a [[RenderImage]] uses as its
  * intrinsic size. */
trait RasterImage:
